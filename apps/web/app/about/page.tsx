...
function OrbitGraphic() {
  return (
    <div className="relative flex h-72 w-72 items-center justify-center sm:h-80 sm:w-80">
      ...
      {/* Center logo */}
      <div className="relative z-10 flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-[#0C5C57] to-[#1a8a82] shadow-lg shadow-teal-900/20">
        <UdeetsLogoIcon className="h-10 w-10 text-white/90" alt="" />
      </div>

      {/* Floating nodes */}
      <div className="absolute left-2 top-8 animate-[float_3s_ease-in-out_infinite] rounded-xl bg-white p-2 shadow-md border border-slate-100">
        <Megaphone className="h-5 w-5 text-[#0C5C57]" />
      </div>
      <div className="absolute right-0 top-1/4 animate-[float_3s_ease-in-out_infinite_0.5s] rounded-xl bg-white p-2 shadow-md border border-slate-100">
        <Users className="h-5 w-5 text-blue-500" />
      </div>
      <div className="absolute bottom-8 left-4 animate-[float_3s_ease-in-out_infinite_1s] rounded-xl bg-white p-2 shadow-md border border-slate-100">
        <Globe className="h-5 w-5 text-purple-500" />
      </div>
      <div className="absolute bottom-1/4 right-2 animate-[float_3s_ease-in-out_infinite_1.5s] rounded-xl bg-white p-2 shadow-md border border-slate-100">
    ...
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        <section className="relative overflow-hidden py-20 sm:py-28">
          <div className="pointer-events-none absolute -top-32 right-1/4 h-[400px] w-[400px] rounded-full bg-[#EAF6F3]/60 blur-[100px]" />
          ...
          <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
            <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
              <div>
                <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[#A9D1CA]/50 bg-[#EAF6F3] px-4 py-1.5">
              ...
              {[
                ...
                <AnimateOnScroll key={item.emoji} delay={i * 150}>
                  <div className="rounded-2xl border border-slate-100 bg-white p-6 text-center">
                    ...
                    <div className="rounded-lg bg-[#EAF6F3] px-3 py-2 text-sm font-medium text-[#0C5C57]">
                      {item.after}
                    </div>
                  </div>
                </AnimateOnScroll>
            ...
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <AnimateOnScroll key={v.title} delay={i * 100}>
                  <div className="group rounded-2xl border border-slate-100 bg-white p-6 transition duration-300 hover:border-[#A9D1CA] hover:shadow-md">
              ...
              {[
                ...
                <AnimateOnScroll key={milestone.date} delay={i * 200}>
                  <div className={cn("relative mb-12 flex items-start gap-6 last:mb-0", i % 2 === 1 ? "sm:flex-row-reverse sm:text-right" : "")}>
                    {/* Dot */}
                    <div className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#0C5C57] to-[#1a8a82] text-xs font-bold text-white shadow-lg shadow-teal-900/20 sm:absolute sm:left-1/2 sm:-translate-x-1/2">
                      {i + 1}
                    </div>
                    {/* Content */}
                    <div className={cn("flex-1 rounded-2xl border border-slate-100 bg-white p-5", i % 2 === 1 ? "sm:mr-16" : "sm:ml-16")}>
                      <div className="mb-1 text-xs font-semibold uppercase tracking-wider text-[#0C5C57]">{milestone.date}</div>
                      <h3 className="text-base font-semibold text-[#111111]">{milestone.title}</h3>
