...
import type { ComposerVariant } from "../../hooks/useDeetComposer";
import { AnnouncementFormFields } from "./ComposerChildPanels";
import {
  ComposerPhotoIcon,
  ComposerEmojiIcon,
...
export function CreateDeetModal({
  composerVariant = "post",
  announcementTitle,
  announcementBody,
  onAnnouncementTitleChange,
  onAnnouncementBodyChange,
  ...
  enterAnnouncementMode: () => void;
  photoInputRef: RefObject<HTMLInputElement | null>;
  onPhotoFilesChange: (event: ChangeEvent<HTMLInputElement>) => void;
  draftText: string;
  ...
  const isAnnouncement = composerVariant === "announcement";
  const canSubmitAnnouncement = announcementTitle.trim().length > 0;
  ...
  }, [draftText, isAnnouncement]);

  const photoPreviewStrip = selectedPhotoPreviews.length > 0 ? (
    ...
    <div className="fixed inset-0 z-[210] flex items-center justify-center bg-[var(--ud-bg-overlay)] p-4">
      <div className="w-full overflow-hidden rounded-xl border border-[var(--ud-border-subtle)] bg-[var(--ud-bg-card)] shadow-lg" style={{ maxWidth: "560px" }}>
        ...
        <div className="flex items-center justify-between border-b border-[var(--ud-border-subtle)] bg-[var(--ud-bg-subtle)] px-5 py-3.5">
          ...
          <h3 className="text-base font-semibold text-[var(--ud-text-primary)]">{isAnnouncement ? "New announcement" : "Write Post"}</h3>
          <button
            type="submit"
            form="deet-form"
            disabled={isSubmitting || (isAnnouncement && !canSubmitAnnouncement)}
            className={cn(BUTTON_PRIMARY, (isSubmitting || (isAnnouncement && !canSubmitAnnouncement)) && "cursor-not-allowed opacity-60")}
          >
            {isSubmitting ? "Posting" : isAnnouncement ? "Publish" : "Post"}
          </button>
        </div>

        <form id="deet-form" className="px-5 py-4" onSubmit={onSubmit}>
          <input ref={photoInputRef} type="file" accept="image/*" multiple onChange={onPhotoFilesChange} className="hidden" tabIndex={-1} aria-hidden />
          ...
          {isAnnouncement ? (
            <div className="space-y-4">
              <AnnouncementFormFields
                ...
                onBodyChange={onAnnouncementBodyChange}
                titleInputRef={announcementTitleInputRef}
                detailsRows={4}
              />
              <div className="rounded-xl border border-[var(--ud-border-subtle)] bg-[var(--ud-bg-card)] p-4">
                <button
                  type="button"
                  disabled={isSubmitting}
                  onClick={() => photoInputRef.current?.click()}
                  className="w-full rounded-2xl border border-dashed border-[var(--ud-brand-primary)] bg-[var(--ud-bg-subtle)] px-4 py-8 text-center text-sm font-medium text-[var(--ud-brand-primary)] transition hover:bg-[var(--ud-bg-subtle)] disabled:opacity-50"
                >
            ...
            <>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-1 text-[var(--ud-text-muted)]">
                ...
                <button type="button" disabled={isSubmitting} onClick={enterAnnouncementMode} className={ACTION_BTN} title="Announcement">
          ...
          {/* Attached items */}
          {!isAnnouncement && attachedItems.length ? (
